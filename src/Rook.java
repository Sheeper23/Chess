...
public class Rook implements Piece {
	...
	public Rook(String color, int r, int c) {
	...
	public void calculateSight(Piece[][] board, boolean isSecondary) {
		...
		for (int i = row+1; i < board.length; i++) {
			...
			else if (!board[i][col].getColor().equals(color)) {
				lineOfSight[i][col] = 1;
				break;
			}
			else {
		...
		for (int j = col+1; j < board.length; j++) {
			...
			else if (!board[row][j].getColor().equals(color)) {
				lineOfSight[row][j] = 1;
				break;
			}
			else {
		...
		for (int k = 0; k < lineOfSight.length; k++) {
			for (int l = 0; l < lineOfSight[0].length; l++) {
				...
				if (lineOfSight[k][l] != 0) {
					...
					moveSimulation[k][l] = new Rook(color, k, l);
					int kingY = 0;
					int kingX = 0;
